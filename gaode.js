module.exports = function (RED) {
    ...
    function gaodeNode(config) {
        ...
        node.on('input', function (msg) {
            ...
            var zoom = config.zoom || msg.payload.zoom
            var width = config.width || msg.payload.width
            var height = config.height || msg.payload.height
            var size = width + '*' + height
            var payload = {}
            var image_map = `https://restapi.amap.com/v3/staticmap?location=${locations}&zoom=${zoom}&size=${size}&markers=mid,,A:${locations}&key=${gaodeKey}`
            node.log(image_map)
            axios({
                method: 'get',
    ...
    function gaodeNodeConvert(config) {
        RED.nodes.createNode(this, config);
        ...
        node.on('input', function (msg) {
            ...
            var longitude = config.longitude || msg.payload.longitude
            var latitude = config.latitude || msg.payload.latitude
            var coordsys = config.coordsys || msg.payload.coordsys
            let locations = `${longitude},${latitude}`
            var payload = {}            
            axios({
                method: 'get',
                url: `http://restapi.amap.com/v3/assistant/coordinate/convert?key=${gaodeKey}&locations=${locations}&coordsys=${coordsys}`
            }).then(function (response) {
                var data = response.data
                var status = data['status']
                if (status != 1) {
                ...
                payload.status = 1                
                msg.payload = payload
                msg.payload.latitude = data['locations'].split(',')[1]
                msg.payload.longitude = data['locations'].split(',')[0]
                msg['data'] = data
                node.send(msg) 
            }).catch(function (error) {
                payload.status = 0
    ...
    RED.nodes.registerType("gaode-convert", gaodeNodeConvert);
